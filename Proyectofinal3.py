...
def mostrar_ticket(producto, precio, cantidad, total):
  ...
  #Ahora, agrega la siguiente línea que manda llamar la función (la primera línea de messagebox.showinfo ya está en el código, solo debes agregar la siguiente:
...
# -------------------------
estilo = ttk.Style()
# (Opcional) Cambiar el tema para que los colores de fondo funcionen mejor:
estilo.theme_use("default")

# Define un estilo nuevo llamado "MiBoton.TButton"
estilo.configure(
    "MiBoton.TButton",
    font=("Arial", 12),
    foreground="black",            # color de la letra
    background="#FFD580",          # color fondo: un naranja claro (hex)
    padding=10
)

# También puedes definir cómo cambia el color cuando está activo (hover) o presionado:
estilo.map(
    "MiBoton.TButton",
    background=[
        ("active", "#FFECB3"),      # más claro cuando pasa el ratón
        ("pressed", "#FFC04D")      # un poco más oscuro cuando se presiona
    ],
    foreground=[
        ("disabled", "gray"),
        ("!disabled", "black")
...
# BOTONES PRINCIPALES
# -------------------------
btn_reg_prod = ttk.Button(ventana, text="Registro de Productos", command=abrir_registro_productos, style="MiBoton.TButton")
btn_reg_prod.pack(pady=10)

btn_reg_ventas = ttk.Button(ventana, text="Registro de Ventas", command=abrir_registro_ventas, style="MiBoton.TButton")
btn_reg_ventas.pack(pady=10)

btn_reportes = ttk.Button(ventana, text="Reportes", command=abrir_reportes, style="MiBoton.TButton")
btn_reportes.pack(pady=10)

btn_acerca = ttk.Button(ventana, text="Acerca de", command=abrir_acerca_de, style="MiBoton.TButton")
